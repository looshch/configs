...
vim.keymap.set('t', '<c-[>', function()
	if vim.api.nvim_win_get_config(0).title ~= nil then
...
local get_bufnrs = function()
	return vim.tbl_filter(
		function(b)
			return vim.fn.buflisted(b) == 1 and
			vim.fn.bufloaded(b) == 1
		end,
...
require 'resession'.setup{buf_filter = function(b)
	if vim.bo[b].buftype == 'terminal' then return false end
