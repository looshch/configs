...
- `<c-u>` — scroll and jump half the screen up ([u]p);

- `N` `<c-u>` — scroll and jump `N` lines up ([u]p);

- `<c-d>` — scroll and jump half the screen down ([d]own);

- `N` `<c-d>` — scroll and jump `N` lines down ([d]own);

- `<c-f>` — scroll and jump a screen forward ([f]orward);

- `N` `<c-f>` — scroll and jump `N` screens forward ([f]orward);

- `<c-b>` — scroll and jump a screen backward ([b]ackward);

- `N` `<c-b>` — scroll and jump `N` screens backward ([b]ackward);

- `<c-y>` — scroll up;

- `N` `<c-y>` — scroll `N` lines up;

- `<c-e>` — scroll down;

- `N` `<c-e>` — scroll `N` lines down;
...
- `surrounding`s: `'`, `"`, `` ` ``, `[`, `(`, `{`, `<`
  - `y` `s` `motion` `surrounding` — add `surrounding` around `motion` ([y]ank
    `[s]urrounding`);
...
Same as in [Normal Mode](#normal-mode).

#### Text Objects

- `a` `n` — around `node` ([a]round `[n]ode`);
- `i` `n` — inside `node` ([i]nside `[n]ode`).
