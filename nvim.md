...
- `surrounding`s: `'`, `"`, `` ` ``, `[`, `(`, `{`, `<`\
  ...
  of `motion`. If you change `surrounding`, you need to specify both the old
  and the new ones, like `c` `s` `motion` `surrounding` `surrounding`.
...
- `<c-\>`/`<c-\><c-n>` — /open _terminal_/exit terminal _mode_/.
